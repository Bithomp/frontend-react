...
// Convert transaction type to human-readable text
export const txTypeToText = (rawTransaction, type = null, capitalize = false) => {
  // Use provided type or extract from transaction
  const transactionType = type || rawTransaction?.TransactionType
  ...
  switch (transactionType.toLowerCase()) {
    ...
    case 'nftokencreateoffer':
      formattedType = 'NFT offer creating'
      break
    case 'uritokenmint':
    ...
    case 'didset':
      ...
      break
    default:
      // Keep the original type for unhandled cases
...
export const processTransactions = (transactions, currentAddress) => {
  ...
  transactions.forEach((transaction, index) => {
    ...
    if (!transaction.txHash && !transaction.tx?.hash) {
      return
    }

    const output = {
      status: transaction.outcome?.result || 'unknown',
      submitter: transaction.address || transaction.tx?.Account,
      alien: false,
      type: transaction.tx?.TransactionType?.toLowerCase(),
      hash: transaction.txHash || transaction.tx?.hash,
      sequence: transaction.sequence || transaction.tx?.Sequence,
      date: null,
      time: null,
      fullTime: null,
      ...
      destination_tag: '',
      source_tag: '',
      myBalanceChanges: null,
      direction: null,
      address: null
    }

    // Handle NFT offer creation special case
    if (transaction.tx?.TransactionType === 'NFTokenCreateOffer') {
      if (transaction.specification?.flags?.sellToken) {
        output.type = 'NFT Sell Offer Creation'
      } else {
        output.type = 'NFT Buy Offer Creation'
      }
    }

    // Process timestamp
    if (transaction.outcome?.timestamp) {
      output.date = transaction.outcome.timestamp.substring(0, 10)
      output.time = transaction.outcome.timestamp.substring(11, 16)
      output.fullTime = transaction.outcome.timestamp.substring(11, 19) + ' (UTC)'
    } else if (transaction.tx?.date) {
      const date = new Date((transaction.tx.date + 946684800) * 1000)
      output.date = date.toISOString().substring(0, 10)
      output.time = date.toISOString().substring(11, 16)
      output.fullTime = date.toISOString().substring(11, 19) + ' (UTC)'
    }
    ...
    if (output.type === 'payment') {
      ...
      if (destination === currentAddress && source === currentAddress) {
        ...
        output.type = 'rippling'
      } else {
        output.type = 'balance change'
      }

      // Add tags and delivered amount for payments
      if (output.type === 'payment') {
        if (transaction.specification?.source?.tag) {
          output.source_tag = transaction.specification.source.tag
        }
      ...
      output.specification = transaction.outcome?.orderbookChanges?.[currentAddress]?.[0] || {}
      output.specification.orderSequence = transaction.specification?.orderSequence
    } else if (output.type === 'order') {
