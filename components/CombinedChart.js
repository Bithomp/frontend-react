...
export default function CombinedPriceChart({ seriesData, colors, yAxis }) {
    ...
    const timeDifference = firstSeriesData.length > 1
        ? new Date(firstSeriesData[firstSeriesData.length - 1][0]).valueOf() - new Date(firstSeriesData[0][0]).valueOf()
        : 0
    ...
    return (
        <>
            <Chart type="line" series={seriesData} options={options} />
        </>
    )
}