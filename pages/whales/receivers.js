...
export async function getServerSideProps(context) {
  ...
  try {
    const res = await axiosServer({
      method: 'get',
      url: 'v2/address/whale/receivers',
...
export default function Whales({ data }) {
  ...
  return (
    <>
      <SEO title={ledgerName + '  addresses that received the most in the last 24h'} />
      <div className="content-text">
        <WhaleTabs tab="receivers" />
        <h1 className="center">{ledgerName + '  addresses that received the most in the last 24h'}</h1>
