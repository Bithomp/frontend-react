...
export default function Payments() {
  ...
  const getApiData = async () => {
    ...
    */
    ...
    setApiPayments({})
    const apiTransactions = await axiosAdmin
      .get('partner/accessToken/transactions?limit=50&offset=0')
      .catch((error) => {
  ...
  const apiPrice = (tier, months = 1) => {
    ...
    return (
      <>
        <b>
          {((tierRate * months) / eurRate).toFixed(2)} {nativeCurrency}
        </b>{' '}
        ({tierRate * months} EUR)
      </>
    )
  ...
  const apiPlanTr = (tierName, months = 1) => {
    return (
      <tr>
        ...
        <td className="left">{apiPrice(tierName, months)}</td>
      </tr>
    )
  }
            ...
            <>
              {apiData && (
                <>
                  ...
                  {width > 500 ? <br /> : ' '}
                  If it's not activated within 24h, please contact us at <b>partner@bithomp.com</b>{' '}
                  <CopyButton text="partner@bithomp.com" />
                </>
              )}
